...
class AdaptiveStatsWriter:
    ...
    def manual_find_thresholds(self, *, base_candidate_low: int, base_file_bias: float, adaptive_tuning: bool) -> tuple[int, float]:
        ...
        last = rows[-1]
        candidate_low = int(last.get("candidate_low_threshold", candidate_low))
        file_bias = float(last.get("file_bias_threshold", file_bias))

        now_ms = int(time.time() * 1000)
        recent_24h = [r for r in rows if now_ms - int(r.get("ts", now_ms)) <= 24 * 60 * 60 * 1000]
        # Spec requires threshold updates to move at most once per 24h.
        can_adjust_now = True
        if recent_24h:
            candidate_values = {int(r.get("candidate_low_threshold", candidate_low)) for r in recent_24h}
            file_bias_values = {round(float(r.get("file_bias_threshold", file_bias)), 2) for r in recent_24h}
        ...
        if len(rows) >= 200:
            ...
            if (prev_recall - curr_recall) > 0.03 or (curr_rate - prev_rate) > 0.05:
                candidate_low = base_candidate_low
                file_bias = base_file_bias
    ...
    def _recall_proxy(rows: list[dict[str, Any]]) -> float:
        ...
        for row in rows:
            if row.get("cutoff_reason"):
                continue
            if int(row.get("candidates", 0)) > 0:
                good += 1
                continue
            if int(row.get("added_evidence_count", 0)) > 0:
                good += 1
